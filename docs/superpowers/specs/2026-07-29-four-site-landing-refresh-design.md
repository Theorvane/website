...
Refresh the public homepages for Theorvane, TypeMCP, TypeChain, and OpenScene. The pages will adopt a shared conversion sequence inspired by the information hierarchy of the NestJS homepage without copying its visual identity, copy, sponsorship model, or interaction patterns. Each application remains an independently deployable Next.js site with its own visual language and product boundary.
