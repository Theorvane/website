...
describe("PlanGuard technical SEO", () => {
	it("configures the approved Rybbit site script", () => {
		...
		expect(script.props).toMatchObject({
			...
			"data-site-id": "0b0d4eec0814",
			defer: true,
		});
	});

	it("publishes canonical and social metadata for the subdomain", () => {
		expect(metadata.metadataBase?.toString()).toBe("https://planguard.theorvane.tech/");
		expect(metadata.alternates?.canonical).toBe("/");
		expect(metadata.applicationName).toBe("PlanGuard");
		...
		expect(metadata.openGraph?.url?.toString()).toBe("https://planguard.theorvane.tech/");
	...
	it("publishes crawl directives and a canonical sitemap", () => {
		...
		expect(robots().sitemap).toBe("https://planguard.theorvane.tech/sitemap.xml");
		expect(robots().host).toBe("https://planguard.theorvane.tech");
		expect(sitemap()).toEqual([{ url: "https://planguard.theorvane.tech/", lastModified: "2026-08-28", changeFrequency: "monthly", priority: 1 }]);
	});
	...
	it("renders SoftwareApplication JSON-LD naming Theorvane as publisher", () => {
		...
		expect(parsed.name).toBe("PlanGuard");
	...
	it("publishes a rendered social preview image at the documented size", () => {
		...
		expect(ogAlt).toContain("PlanGuard");
	});
});
