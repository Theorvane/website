...
export function generateStaticParams() { return documentEditions().map((edition)=>({ slug:edition.document.route.replace("/docs/","").split("/") })); }
export async function generateMetadata({ params }: { params: Promise<{ slug: string[] }> }): Promise<Metadata> { const route=`/docs/${(await params).slug.join("/")}`; const document=await getDocument(route); if (!document) return {}; const title=`${document.title} | TypeChain`; const description=document.document.summary; return { title, description, alternates:{canonical:route,languages:localeAlternates(route,document.locale)}, openGraph:{type:"article",title,description,url:`${canonicalBase}${route}`,siteName:"TypeChain",locale:openGraphLocale(document.locale)}, twitter:{card:"summary_large_image",title,description}}; }
/**
...
function documentSchema(route: string, title: string, summary: string, locale: DocumentLocale) { const url=`${canonicalBase}${route}`; return { "@context":"https://schema.org", "@graph":[
...
export default async function DocumentPage({ params }: { params: Promise<{ slug: string[] }> }) { const route=`/docs/${(await params).slug.join("/")}`; const [documents,document]=await Promise.all([getAllDocuments(documentLocale(route)),getDocument(route)]); if(!document) notFound(); return <><a className="skip-link" href="#docs-content">Skip to documentation</a><script data-testid="typechain-docs-schema" type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(documentSchema(route, document.title, document.document.summary, document.locale)) }} /><header className="docs-header"><a className="logo" href="/">TYPE<span>CHAIN</span></a><nav aria-label="Primary"><a href="/">Product</a><a href="/docs">Documentation</a><a href="https://github.com/Theorvane/type-chain" target="_blank" rel="noopener noreferrer">GitHub</a></nav></header><main className="docs-layout" lang={document.locale}><DocsSidebar documents={documents} activeRoute={route}/><MarkdownArticle document={document}/><DocumentPager documents={documents} route={route}/><ArticleToc document={document}/></main></>; }
