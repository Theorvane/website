...
export async function getDocument(route: string): Promise<RepositoryDocument | undefined> {
  ...
  return { document: edition.document, locale: edition.locale, markdown, sourceUrl: `https://github.com/Theorvane/type-chain/blob/${sourceCommit}/${edition.sourcePath}`, ...parseDocument(markdown, edition.document, edition.sourcePath) };
}
