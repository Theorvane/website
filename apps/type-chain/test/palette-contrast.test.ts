 ...
 */
describe("TypeChain palette contrast", () => {
	it("keeps hero-derived accent text legible on the page background", async () => {
		const { "--bg": bg, "--accent": accent, "--ink": ink, "--muted": muted } = await tokens();
		expect(contrast(accent as string, bg as string)).toBeGreaterThanOrEqual(4.5);
	...
	it("keeps white button and skip-link text legible on the accent", async () => {
		const { "--accent": accent } = await tokens();
		expect(contrast("#ffffff", accent as string)).toBeGreaterThanOrEqual(4.5);
	});
