...
describe("OpenVideo documentation routes", () => {
	...
	it("renders the English index with a group listing", async () => {
		...
		expect(main.getByRole("heading", { level: 1, name: "OpenVideo documentation" })).toBeDefined();
	...
	it("publishes per-page canonical and hreflang metadata", async () => {
		...
		expect(metadata.openGraph?.url).toBe("https://open-video.app/docs/ko/install");
	...
	it("renders TechArticle and breadcrumb JSON-LD for a page", async () => {
		await renderRoute(["install"]);
		const graph = JSON.parse(screen.getByTestId("openvideo-docs-schema").textContent ?? "{}") as { "@graph"?: Array<Record<string, unknown>> };
		expect(graph["@graph"]).toEqual(
			expect.arrayContaining([
				expect.objectContaining({ "@type": "TechArticle", url: "https://open-video.app/docs/install", inLanguage: "en" }),
