...
export const koStrings: LocaleStrings = {
	indexTitle: "OpenVideo 문서",
	indexSummary: "소스에서 OpenVideo를 실행하고, 워크스페이스와 타임라인을 익히고, Edit Agent를 활용하고, 원하는 모델 프로바이더만 연결하는 방법을 다룹니다.",
	indexLede: "OpenVideo는 현재 소스에서 실행합니다. 패키징된 설치 파일은 아직 없습니다. 이 문서에 적힌 내용은 모두 지금 빌드에 실제로 있는 동작이며, 예정된 작업은 아직 제공하지 않는다고 명시합니다.",
	sidebarLabel: "문서",
	onThisPage: "이 페이지 내용",
	previous: "이전",
	next: "다음",
...
const overview: DocPage = {
	...
	blocks: [
		{ kind: "paragraph", text: "OpenVideo는 내 컴퓨터에서 영상을 편집하는 오픈소스 Electron 애플리케이션입니다. 폴더를 하나 열어 프로젝트로 삼고, 타임라인에 클립을 올리고, 시스템에 이미 설치된 FFmpeg로 H.264/AAC MP4를 내보냅니다." },
		...
		{ kind: "note", tone: "caution", text: "OpenVideo는 정식 출시 전입니다. 소스에서 실행하며 패키징된 설치 파일과 자동 업데이트가 없습니다. [설치와 실행](/docs/ko/install)을 참고하세요." },
...
const install: DocPage = {
	...
	blocks: [
		{ kind: "note", tone: "caution", text: "패키징된 설치 파일과 자동 업데이트는 아직 없습니다. OpenVideo는 소스에서 실행하며, 이 문서는 그 사실이 바뀔 때에만 다르게 적힐 것입니다." },
		{ kind: "heading", text: "사전 요구사항" },
		{ kind: "list", items: [
			"Node.js 22 이상, npm 10 이상",
			"FFmpeg — MP4 내보내기에 필요합니다",
			"macOS 한정: 창 캡처를 쓴다면 OpenVideo를 실행하는 터미널에 화면 기록 권한이 필요합니다",
		] },
		...
		{ kind: "paragraph", text: "OpenVideo는 자체 FFmpeg를 포함하지 않고 **사용자가 설치한** FFmpeg를 구동합니다. `PATH`의 절대 경로 디렉터리를 통해 `ffmpeg`를 찾을 수 있게 하거나, 바이너리를 명시적으로 지정하세요." },
		{ kind: "code", language: "bash", lines: ["VIDEO_TOOL_FFMPEG_PATH=/absolute/path/to/ffmpeg npm run dev"] },
		{ kind: "note", tone: "info", text: "상대 경로 FFmpeg는 거부됩니다. 사용 가능한 FFmpeg가 없으면 OpenVideo는 끝내지 못할 내보내기를 시작하지 않고 문제를 알립니다." },
...
const projects: DocPage = {
	...
	blocks: [
		{ kind: "paragraph", text: "OpenVideo에는 프로젝트 데이터베이스도, 숨겨진 라이브러리도 없습니다. 폴더를 지정하면 그 폴더가 프로젝트입니다. 가져온 에셋은 그 안으로 복사되고, 에이전트 대화도 그 안에 저장되며, 완료된 생성 결과도 그 안에 놓입니다." },
		...
		{ kind: "paragraph", text: "프로젝트가 평범한 폴더이기 때문에 백업, 다른 컴퓨터로 이동, 버전 관리 등록은 모두 사용자의 선택이고 OpenVideo가 관여할 필요가 없습니다." },
	],
};
...
const workspace: DocPage = {
	...
	blocks: [
		...
		{ kind: "figure", image: "editor", alt: "미디어 빈, 프로그램 모니터, 인스펙터, 타임라인, Edit Agent 채팅 패널로 구성된 OpenVideo 편집 워크스페이스", caption: "편집 화면 — 왼쪽에 프로젝트·미디어 독, 가운데에 Program Monitor와 인스펙터, 아래에 타임라인, 오른쪽에 Edit Agent." },
...
const exportPage: DocPage = {
	title: "내보내기",
	summary: "OpenVideo가 내 FFmpeg로 MP4를 렌더링하는 방식과, 시작하지 않는 경우.",
	blocks: [
		{ kind: "paragraph", text: "내보내기는 사용자 컴퓨터의 FFmpeg 바이너리를 구동해 H.264/AAC MP4를 만듭니다. OpenVideo는 자체 FFmpeg를 포함하지 않고, 타임라인을 업로드하지 않으며, 클라우드에서 렌더링하지 않습니다." },
		...
		{ kind: "note", tone: "caution", text: "사용 가능한 FFmpeg가 없으면 OpenVideo는 완료할 수 없는 내보내기를 시작하지 않고 문제를 알립니다. 상대 경로는 거부됩니다. [트러블슈팅](/docs/ko/troubleshooting)을 참고하세요." },
...
const videoGeneration: DocPage = {
	...
	blocks: [
		...
		{ kind: "paragraph", text: "생성 비용은 프로바이더가 사용자에게 직접 청구하며, OpenVideo가 그 사이에 끼어들지 않습니다. 에이전트가 `createVideoJob`으로 작업을 시작할 때 먼저 승인을 요청하는 이유도 같습니다." },
	],
};
...
const dataAndPrivacy: DocPage = {
	...
	blocks: [
		{ kind: "paragraph", text: "OpenVideo의 프라이버시 특성은 어딘가에서 찾아 켜야 하는 설정이 아니라 구조에서 나옵니다." },
...
const troubleshooting: DocPage = {
	...
	blocks: [
		...
		{ kind: "paragraph", text: "OpenVideo는 렌더링 전에 FFmpeg를 확인하고, 중간에 실패하는 대신 문제를 알립니다. `PATH`의 **절대 경로** 디렉터리에서 바이너리를 찾을 수 있는지 확인하거나 명시적으로 지정하세요. 상대 경로는 거부됩니다." },
		{ kind: "code", language: "bash", lines: ["VIDEO_TOOL_FFMPEG_PATH=/absolute/path/to/ffmpeg npm run dev"] },
		...
		{ kind: "paragraph", text: "macOS는 OpenVideo를 실행하는 터미널에 화면 기록 권한을 요구합니다. 시스템 설정에서 권한을 준 뒤 앱을 다시 시작하세요. 전체 화면 캡처와 레코더의 마이크·시스템 오디오 믹스는 아직 제공하지 않습니다." },
		...
		{ kind: "paragraph", text: "`npm run typecheck`, `npm test`, `npm run build`로 깨끗한 빌드에서 재현한 뒤, 플랫폼·Node 버전·FFmpeg 버전을 적어 [github.com/Theorvane/openvideo/issues](https://github.com/Theorvane/openvideo/issues)에 이슈를 열어 주세요." },
	],
};
