...
describe("LabFox technical SEO", () => {
	it("configures the approved Rybbit site script", () => {
		...
		expect(script.props).toMatchObject({
			...
			"data-site-id": "dd7fd8c3ae1a",
			defer: true,
		});
	});

	it("publishes canonical and social metadata for the subdomain", () => {
		expect(metadata.metadataBase?.toString()).toBe("https://labfox.theorvane.tech/");
		expect(metadata.alternates?.canonical).toBe("/");
		expect(metadata.applicationName).toBe("LabFox");
		...
		expect(metadata.openGraph?.url?.toString()).toBe("https://labfox.theorvane.tech/");
	...
	it("publishes crawl directives and a canonical sitemap", () => {
		...
		expect(robots().sitemap).toBe("https://labfox.theorvane.tech/sitemap.xml");
		expect(robots().host).toBe("https://labfox.theorvane.tech");
		expect(sitemap()).toEqual([{ url: "https://labfox.theorvane.tech/", lastModified: "2026-08-28", changeFrequency: "monthly", priority: 1 }]);
	});
	...
	it("renders SoftwareApplication JSON-LD naming Theorvane as publisher", () => {
		...
		expect(parsed.name).toBe("LabFox");
