...
const install: DocPage = {
	...
	blocks: [
		...
		{ kind: "list", items: [
			...
			"macOS only: Screen Recording permission for the terminal running OpenScene, if you use window capture",
		] },
		...
		{ kind: "paragraph", text: "OpenScene drives the FFmpeg **you** installed rather than bundling its own. Either make `ffmpeg` discoverable through an absolute directory on `PATH`, or name the binary explicitly:" },
		{ kind: "code", language: "bash", lines: ["VIDEO_TOOL_FFMPEG_PATH=/absolute/path/to/ffmpeg npm run dev"] },
		{ kind: "note", tone: "info", text: "Relative FFmpeg paths are rejected. Without a usable FFmpeg, OpenScene reports the problem instead of starting an export it cannot finish." },
...
const projects: DocPage = {
	...
	blocks: [
		{ kind: "paragraph", text: "OpenScene has no project database and no hidden library. You point it at a folder, and that folder is the project. Assets you import are copied into it, agent conversations are stored in it, and finished generations land in it." },
		...
		{ kind: "paragraph", text: "Because a project is an ordinary folder, backing it up, moving it to another machine, or putting it in version control is your decision and needs nothing from OpenScene." },
	],
};
...
const workspace: DocPage = {
	...
	blocks: [
		...
		{ kind: "figure", image: "editor", alt: "The OpenScene editing workspace: media bin, program monitor, inspector, timeline, and the Edit Agent chat panel", caption: "The editing surface — project and media dock on the left, Program Monitor and inspector in the middle, timeline along the bottom, Edit Agent on the right." },
...
const exportPage: DocPage = {
	title: "Exporting",
	summary: "How OpenScene renders an MP4 through your own FFmpeg, and what it refuses to start.",
	blocks: [
		{ kind: "paragraph", text: "Export produces an H.264/AAC MP4 by driving the FFmpeg binary on your machine. OpenScene does not bundle its own, does not upload your timeline, and does not render in the cloud." },
		...
		{ kind: "note", tone: "caution", text: "Without a usable FFmpeg, OpenScene reports the problem rather than starting an export it cannot complete. Relative FFmpeg paths are rejected. See [Troubleshooting](/docs/troubleshooting)." },
...
const videoGeneration: DocPage = {
	...
	blocks: [
		...
		{ kind: "paragraph", text: "Your provider bills you directly for generation; OpenScene never sits between you and them. When the agent starts a job with `createVideoJob` it asks for approval first for the same reason." },
	],
};
...
const dataAndPrivacy: DocPage = {
	...
	blocks: [
		{ kind: "paragraph", text: "OpenScene's privacy properties are structural rather than a setting you have to find and switch on." },
...
const troubleshooting: DocPage = {
	...
	blocks: [
		...
		{ kind: "paragraph", text: "OpenScene checks FFmpeg before rendering and reports the problem instead of failing halfway. Confirm the binary is discoverable through an **absolute** directory on `PATH`, or name it explicitly — relative paths are rejected:" },
		{ kind: "code", language: "bash", lines: ["VIDEO_TOOL_FFMPEG_PATH=/absolute/path/to/ffmpeg npm run dev"] },
		...
		{ kind: "paragraph", text: "macOS requires Screen Recording permission for the terminal running OpenScene. Grant it in System Settings and restart the app. Full-screen capture, and microphone or system-audio mix in the recorder, are not available yet." },
		...
		{ kind: "paragraph", text: "Reproduce from a clean build with `npm run typecheck`, `npm test`, and `npm run build`, then open an issue at [github.com/Theorvane/openscene/issues](https://github.com/Theorvane/openscene/issues) with your platform, Node version, and FFmpeg version." },
	],
};
