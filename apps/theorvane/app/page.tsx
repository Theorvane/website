	...
	return <>
		...
		<header className="shell">
			...
			<nav aria-label="Primary">
				...
				<ExternalLink href={github}>GitHub organization ↗</ExternalLink>
			</nav>
		</header>
		<main id="main-content">
			<section className="hero shell" id="top">
				...
				<div className="actions">
					<ExternalLink className="button primary" href={typeMcp}>Explore TypeMCP ↗</ExternalLink>
					<ExternalLink className="button" href={github}>GitHub organization ↗</ExternalLink>
				</div>
			</section>
			<section className="signals shell" aria-label="What Theorvane values">
			...
			<section className="feature shell" id="projects">
				...
				<div className="feature-grid">
					<div>
						<h2>TypeMCP</h2>
						<p>A decorator-first TypeScript toolkit for MCP metadata declarations and immutable reads.</p>
						<ExternalLink href={typeMcp}>View the project ↗</ExternalLink>
					</div>
					<pre aria-label="TypeMCP code preview"><code>{`@McpTool({ name: "status" })
status() {
  return { ready: true };
}`}</code></pre>
				</div>
			...
			<section className="feature shell">
				...
				<div className="feature-grid">
					...
					<pre aria-label="OpenVideo local workflow preview"><code>{`CAPTURE  →  EDIT  →  EXPORT

Your footage stays on your device.`}</code></pre>
				</div>
			</section>
			<section className="principles shell" id="principles">
				...
				<div className="principle-grid">
					...
					<article><h2>Small, proven releases</h2><p>We prefer a narrow surface area backed by real checks over an ambitious promise.</p></article>
				</div>
			</section>
		</main>
		<footer className="shell">
			<p>© {new Date().getFullYear()} Theorvane. Built with intent.</p>
			<ExternalLink href={github}>GitHub organization ↗</ExternalLink>
		</footer>
	</>;
}
